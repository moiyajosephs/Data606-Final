...
As shown in the bar charts above there are 5 values for the `goout` variable where 5 indicated that the student goes out very regularly. The most common answer is 3 within the group and it is also the group where mosr students do not have a romantic relationship. The meduan grade for the students who go out with a value of 3 is around 10. 
...
y =  11.00617 - 1.21809 * romanticyes +  0.19663 * freetime -0.62747 * goout + 0.03788 * Walc + 0.24075 * famrel + activitiesyes * 0.18516

The model has an adjusted R squared of 0.024. 
...
```{r}
robust.model.full <- rlm(G3 ~ romantic + freetime + goout + Walc + famrel + activities, data= student_mat)
...
```{r}
robust.model.reduced <- rlm(G3 ~ romantic + goout, data=student_mat)
...
```{r}
model.full <- lm(G3 ~ romantic + freetime + goout + Dalc + Walc + famrel + activities, data= student.pass)
summary(model.full)
vif(model.full)
```
### Linear Model

Now without the outliers only `goout` is the only statistically significant variable.


```{r}
model_reduced <- lm(G3 ~ goout , data= student.pass)
...
ggplot(data = model_reduced, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_jitter() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
...
```{r}
ggplot(model_reduced, aes(sample = .resid)) +
 stat_qq() +
    stat_qq_line() +
  xlab("resid")
```
...
ggplot(model_reduced, aes(x=.resid)) +
