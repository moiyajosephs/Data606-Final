...
```{r}
ggplot(student_mat, aes(G3)) + geom_bar()
```

Normally distributed with one outlier
...
ideal.students <- tibble(predict(ideal.student.model))
ideal.students
```
...
By using the rlm model it is much lower. 
